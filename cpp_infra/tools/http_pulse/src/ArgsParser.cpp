...
void build_po(nvd::ProgramOptions& options)
{
    ...
    options.add_arg<bool>("dry_run,dr", "", "dry_run. Run single test request (default: false)");
...
std::optional<nvd::HttpCommand> parseHttpParams(int argc, char** argv)
{
    ...
    if (!host || !target || !num_connections || !port || !version || !metrics_out_path || !name || !dry_run) return std::nullopt;
    return nvd::HttpCommand{*host, *port, *target, *num_connections, *version, *metrics_out_path, *name, *dry_run, runtime_seconds, user, token, cert_path, body};
}
