...
import subprocess
import logging
from .utils_constants import ShellCommands
import shutil
...
def run_command(command, device=None, verbose=True, timeout=None, no_prefix=True,username:str ="root",password:str=""):
    """
    ...
    if not is_localhost(device):
        ...
        try:
            conn = connect_to_remote(device, username, '')
        except Exception:
            connection_error = True

        if not conn:
            conn = connect_to_remote(device, username, password)
            connection_error = False
...
def run_ssh_command(command, device=None, verbose=True, username:str="root"):
    ...
    if not is_localhost(device):
        ...
        # the shell commands needs those items {username}@{device} '{command}'
        if ':' in device:
            command = ShellCommands.SSH_CMD_IPV6.format(username,device,command)
        else:
            command = ShellCommands.SSH_CMD.format(username,device,command)
...
def is_folder_exists(remote_path:str, server:str,username:str,password:str)->bool:
